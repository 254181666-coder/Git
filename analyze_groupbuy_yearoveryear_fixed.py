...
def load_this_year_data():
    ...
    # 只加载到2026-05-11的数据，与去年同期5.1-5.11对齐
    od = query("""
...
<title>团购同比分析报告 - 2025年5月1-11日vs2026年5月1-11日</title><style>
...
<h1>📈 团购同比分析报告(5月1-11日对齐)</h1>
<div style="color:#7f8c8d;margin-bottom:20px;font-size:13px;">
对比周期：<b>2025年5月1-11日</b> vs <b>2026年5月1-11日</b><br>
说明：修正了时间对齐问题，去年和今年都只取5月前11天的数据进行对比
</div>
"""
    
    ly_total_orders = sum([last_year_data[s]['团购订单'] for s in last_year_data if s in last_year_data])
    ...
    ty_total_orders = sum([this_year_data[s]['团购订单'] for s in this_year_data if s in this_year_data])
    ty_total_rev = sum([this_year_data[s]['团购营收'] for s in this_year_data if s in this_year_data])
    ...
    html += '<tr><th>指标</th><th>2025年5月1-11日</th><th>2026年5月1-11日</th><th>变化幅度</th></tr>'
    ...
    html += '<tr><th>门店</th><th colspan="4">2025年5月1-11日</th><th colspan="4">2026年5月1-11日</th><th colspan="2">同比变化</th></tr>'
    html += '<tr><th></th><th>总订单</th><th>美团</th><th>抖音</th><th>总营收</th><th>总订单</th><th>美团</th><th>抖音</th><th>团购营收</th><th>订单变化</th><th>营收变化</th></tr>'
    
    for store in stores:
        ly = last_year_data.get(store, {'团购订单': 0, '美团订单': 0, '抖音订单': 0, '总营收': 0})
        ty = this_year_data.get(store, {'总订单': 0, '美团订单': 0, '抖音订单': 0, '团购营收': 0})
        
        ly_orders = ly.get('团购订单', ly.get('总订单', 0))
        ly_rev = ly.get('总营收', 0)
        ty_orders = ty.get('团购订单', 0)
        ty_rev = ty.get('团购营收', 0)
        
        ord_chg = (ty_orders - ly_orders) / ly_orders * 100 if ly_orders > 0 else 0
        rev_chg = (ty_rev - ly_rev) / ly_rev * 100 if ly_rev > 0 else 0
        
        ord_cls = 'pos' if ord_chg >= 0 else 'neg'
        rev_cls = 'pos' if rev_chg >= 0 else 'neg'
        
        html += f"<tr><td><b>{store}</b></td>"
        html += f"<td>{ly.get('团购订单', ly.get('总订单', 0)):,.0f}</td><td>{ly.get('美团订单', 0):,.0f}</td><td>{ly.get('抖音订单', 0):,.0f}</td><td>{ly.get('总营收', 0):,.0f}</td>"
        html += f"<td>{ty.get('总订单', 0):,.0f}</td><td>{ty.get('美团订单', 0):,.0f}</td><td>{ty.get('抖音订单', 0):,.0f}</td><td>{ty.get('团购营收', 0):,.0f}</td>"
        html += f"<td><span class=\"{ord_cls}\">{pct(ty_orders, ly_orders)}</span></td>"
        html += f"<td><span class=\"{rev_cls}\">{pct(ty_rev, ly_rev)}</span></td>"
        html += "</tr>"
...
对比周期：2025年5月1-11日vs2026年5月1-11日 | 数据来源：Excel+数据库 | 糖果华庭 KTV
</div></div></body></html>"""
    
    output_path = OUTPUT_DIR / "团购同比分析报告_2025vs2026_对齐版.html"
    ...
    pdf_path = pdf_dir / "团购同比分析报告_2025vs2026_对齐版.pdf"
...
def main():
    print("=" * 80)
    print("团购同比分析 - 2025年5月1-11日 vs 2026年5月1-11日(时间对齐)")
    print("=" * 80)
    print()
    ...
    print()
    print("📊 数据加载完成(已对齐时间)")
    print(f"  - 去年5月1-11日有数据门店: {len([s for s in last_year_data if last_year_data[s]['团购订单'] > 0])}")
    print(f"  - 今年5月1-11日有数据门店: {len([s for s in this_year_data if this_year_data[s]['团购订单'] > 0])}")
    
    print("\n=== 调试输出：各门店数据 ===")
    for store in ['佳木斯', '安达']:
        if store in last_year_data:
            ly = last_year_data[store]
            print(f"\n{store} 2025:")
            print(f"  总订单: {ly.get('团购订单', 0)}")
            print(f"  美团订单: {ly.get('美团订单', 0)}, 抖音订单: {ly.get('抖音订单', 0)}")
            print(f"  总营收: {ly.get('总营收', 0):,.0f}")
        
        if store in this_year_data:
            ty = this_year_data[store]
            print(f"\n{store} 2026:")
            print(f"  总订单: {ty.get('团购订单', 0)}")
            print(f"  总营收: {ty.get('团购营收', 0):,.0f}")
