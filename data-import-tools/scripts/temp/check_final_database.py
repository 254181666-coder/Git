...
def main():
    ...
    # 检查 order_detail
    print("\n📊 order_detail:")
    cursor.execute("SELECT data_date, COUNT(*) FROM order_detail GROUP BY data_date ORDER BY data_date DESC")
    for row in cursor.fetchall():
        print(f"   {row[0]}: {row[1]} 条")

    # 检查 order_daily
    print("\n📊 order_daily:")
    cursor.execute("SELECT data_date, COUNT(*) FROM order_daily GROUP BY data_date ORDER BY data_date DESC")
    for row in cursor.fetchall():
        print(f"   {row[0]}: {row[1]} 条")
    ...
    # 检查 product_sales_detail 的分类信息
    print("\n🎯 product_sales_detail 分类统计:")
    cursor.execute("""
