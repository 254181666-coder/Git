...
def main():
    ...
    excluded_keywords = ['打印纸', '小票', 'pos', '三联', 'A4']  # 排除打印纸
    ...
    for i in range(3, len(df_spare)):
        ...
        raw_products.append({
            ...
            'store_out': store_out,
            'note': note,
            'is_assessment': '不算考核' not in note  # 是否考核项
        })
    
    # 输出产品清单供确认
    print("处理的产品清单:")
    for i, p in enumerate(raw_products):
        status = "✓考核项" if p['is_assessment'] else "✗不考核"
        print(f"  {i}: {p['product_name']} ({status}) - {p['note']}")
    print("\n")
    
    # 合并两种一次性马桶垫
    mattress_no_woven = None  # 无纺布
    mattress_soluble = None   # 溶水型
    ...
    if mattress_no_woven and mattress_soluble:
        ...
        print(f"✓ 已合并两种一次性马桶垫: 无纺布 + 溶水型\n")
    
    # 构建最终产品列表：只保留考核项，并合并马桶垫
    final_products = []
    ...
    for p in raw_products:
        ...
        if not p['is_assessment']:
            print(f"✗ 已排除不考核项: {p['product_name']}")
            continue
    ...
    for p in final_products:
        ...
        for store_name in stores_data.keys():
            ...
            total_cust = customers_map.get(store_name, 0)
            per_cust = out_value / total_cust if total_cust > 0 else 0
            per_cust_pct = f'{per_cust * 100:.2f}%'
            
            cells.append(f'<td class="number">{out_value}</td>')
            cells.append(f'<td class="number">{int(total_cust)}</td>')
            cells.append(f'<td class="number">{per_cust_pct}</td>')
    ...
    for store_name in stores_data.keys():
        ...
        unqualified_html.append('<th style="border:1px solid #ddd;padding:10px;">{0}每客消耗</th>'.format(store_name))
...
<body>
    <div class="container">
        ...
        <p class="note">已排除：不考核项、打印纸类；两种一次性马桶垫已合并计算</p>
        <table>
    ...
    html_output_file = Path('report_v4.html')
    ...
    print(f"\n处理完成！")
    print(f"  HTML报告: {html_output_file}")
    print(f"  Excel报告: {excel_output_file}")
