...
for idx, row in df_douyin.iterrows():
    # 检查是否是门店名称行 (列0有值且不是标题)
    store_name = row.iloc[0]
    if pd.notna(store_name) and str(store_name).strip() not in ['商铺名称', '']:
        current_store = str(store_name).strip()
        continue
    
    # 如果不是标题行，且有数据，则解析套餐
    if current_store and pd.notna(row.iloc[1]) and str(row.iloc[1]).strip() not in ['序号', '']:
        pkg_info = {
            'store': current_store,
            'platform': '抖音',
            ...
            'drink': str(row.iloc[6]) if pd.notna(row.iloc[6]) else '/',
            'snack': str(row.iloc[7]) if pd.notna(row.iloc[7]) else '/',
            'time': str(row.iloc[10]) if pd.notna(row.iloc[10]) else '',
            'sales': str(row.iloc[12]) if pd.notna(row.iloc[12]) else ''
        }
        
        # 构建套餐名称
        pkg_info['name'] = f"抖音{pkg_info['price']}元"
...
for idx, row in df_meituan.iterrows():
    ...
    if current_store and pd.notna(row.iloc[1]) and str(row.iloc[1]).strip() not in ['序号', '']:
        ...
        pkg_info['name'] = f"美团{pkg_info['price']}元"
        packages_2025.append(pkg_info)
...
for pkg in packages_2025:
    ...
    <tr>
        ...
        <td>{pkg['beer']}</td>
        <td>{pkg['drink']}</td>
        <td style="max-width:250px;font-size:11px;">{pkg['snack']}</td>
        <td style="font-size:10px;">{pkg['time']}</td>
        <td>{pkg['sales']}</td>
    </tr>
"""
